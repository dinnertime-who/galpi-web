...
/* ─── Main Screen ─────────────────────────────────────────── */
function MainScreen({ onRecord }: { onRecord: () => void }) {
  return (
    <div className="flex flex-col h-full bg-[#f0eee9]">
      {/* Quote — vertically centered in available space */}
      ...
      <div className="px-6 pb-6 flex flex-col items-center gap-3">
        ...
        >
          <PencilLineIcon size={18} weight="regular" color="#1a3a6b" />
          오늘의 갈피 기록하기
        </button>
    ...
    <div className="flex flex-col h-full bg-[#f0eee9]">
      ...
      <div className="bg-white border-t border-[#e5e4e1] px-6 pt-4 pb-10 flex flex-col gap-4">
        ...
        <button
          type="button"
          onClick={onBack}
          className="w-full h-[52px] rounded-full border border-[#e5e4e1] bg-white flex items-center justify-center text-[#6d6c6a] text-[14px] font-medium transition-colors active:bg-[#f5f5f3]"
        >
          돌아가기
        </button>
      </div>
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-[#e8e5de]">
      <div className="relative w-full h-dvh max-h-[852px] overflow-hidden shadow-2xl">
        {view === "main" && (
          <MainScreen onRecord={() => setView("drawer")} />
        )}
        {view === "drawer" && (
          <DrawerScreen
